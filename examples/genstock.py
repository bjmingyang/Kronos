...
Config = {
    ...
    "SYMBOL": '600977',  # Extracted from CSV filename for display purposes
    "INTERVAL": '5min',  # Data interval (5 minutes for A-share example)
    "HIST_POINTS": 400,  # Lookback historical points (adapted from second code)
    "PRED_HORIZON": 120,  # Prediction horizon (adapted from second code)
    "N_PREDICTIONS": 30,  # Number of probabilistic samples (from first code, can set to 1 for deterministic)
...
def make_prediction(df, predictor):
    ...
    with torch.no_grad():
        ...
        close_preds_main, volume_preds_main = predictor.predict(
            ...
            pred_len=Config["PRED_HORIZON"], T=1.0, top_p=0.9,  # top_p from second code
...
def create_plot(hist_df, close_preds_df, volume_preds_df):
    ...
    ax1.set_title(f'{Config["SYMBOL"]} Probabilistic Price & Volume Forecast (Next {Config["PRED_HORIZON"]} Steps)', fontsize=16, weight='bold')
    ...
    ax2.bar(hist_time, hist_df['volume'], color='skyblue', label='Historical Volume', width=width)
